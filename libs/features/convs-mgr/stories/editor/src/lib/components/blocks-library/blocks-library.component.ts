...
import {
  ...
  VoiceMessageBlock, VideoMessageBlock, ListMessageBlock, JumpBlock, MultipleInputMessageBlock, FailBlock, ImageInputBlock, LocationInputBlock, AudioInputBlock, OpenEndedQuestionBlock
...
export class BlocksLibraryComponent implements OnInit {
  ...
  blockTemplates: StoryBlock[] = [
    ...
    { id: 'open-ended-question-block', type: StoryBlockTypes.OpenEnded, message: 'OpenEnded Question', blockIcon:this.getBlockIcon(StoryBlockTypes.OpenEnded) } as OpenEndedQuestionBlock,
  ];
  ...
  addBlock(type: number) {
    switch (type) {
      ...
      case StoryBlockTypes.OpenEnded:
        this.frame.newBlock(StoryBlockTypes.OpenEnded);
        break;  
    }
  }
