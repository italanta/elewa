...
export class AssessmentFailedSectionComponent implements OnInit
{
  ...
  constructor( private _assessmentScoreService: SetAssessmentScoreService,
  ...
  handleFeedback()
  {
    ...
    for (let i = 0; i < this.assessmentFormArray.length; i++) {
      const question = this.assessmentFormArray.at(i) as FormGroup; 
      // Fetch an option that has been picked as the answer, then compare the id f the selected option to the available options
      ...
      // Calculate percentage and savenit to scre service
